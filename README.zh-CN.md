...
*上面的个人说明是本项目中唯一由人类书写的部分。*

---

AIcoding Governance Kit 用来减少 AI agent 写代码时最容易出现的混乱：重复脚本、
旧版本被重新拿来用、交接说不清楚、危险文件误入 Git，以及影响生产/运行时的
操作没有留下证据。

它通过本地 hooks、Git 守卫、收口工具和 agent skill 实现。目标很简单：让 agent
继续快速推进，但在事情变得不可追溯之前，要求它先搜索、尽量复用、解释清楚，并
留下证据。
...
| 能力 | 解决的问题 |
| --- | --- |
| 🔎 **新增脚本前先搜索** | 生命周期 hooks 会在 agent 新建 `train_*`、`build_*`、`run_*`、`audit_*` 这类任务脚本前，提醒它附近是否已有相似脚本，减少版本爆炸、重复劳动，以及“到底哪个脚本才是真的入口”的混乱。 |
| 🧭 **会话恢复后不丢风险状态** | 如果 session 中断后恢复，之前记录的高影响操作仍然保持高影响，直到完成收口。 |
| 📋 **把脚本归属带进会话** | 如果仓库里有 `scripts/MANIFEST.md`，AGK 会把它复制到本次 session 的 scratch 区，让 agent 一开始就看到已有脚本用途和不要重复造轮子的提示。 |
| 🚦 **区分普通编辑和高影响操作** | 普通源码/文档编辑保持 Git-first；数据库写入、service 变更、部署、受保护产物、备份、跨机器同步等操作需要更强证据。 |
| 🛡️ **阻止常见提交事故** | Git pre-commit 守卫会阻止受保护产物、大型运行时文件、疑似秘钥和项目 smoke-check 失败进入 Git 历史。 |
| 🤝 **让交接明确** | 收口工具和可复用 skill 会要求 agent 用稳定方式说明改了什么、验证了什么、还有什么需要人接手。 |
