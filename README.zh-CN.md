...
Agent Governance Kit 是一个面向 AI 编码 agent 的轻量级治理层。它把
...
Agent Governance Kit 就是为这些构建者准备的。它给 AI 编码工作流加入少量
纪律：
...
安装器会把 Agent Governance Kit 的 hook 组合并进已有 `hooks.json`，而不是
...
Agent Governance Kit 是一个本地防护栏。它用于减少可避免的错误，不是安全
边界。
